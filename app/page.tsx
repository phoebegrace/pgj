...
const testimonials = [
  {
    quote:
      "Although our time working together was brief, we truly appreciated the opportunity to collaborate with Phoebe. She consistently demonstrated intelligence, initiative, and a strong work ethic. Taking a chance on her was an easy decision — she’s sharp, capable, and driven. We’re confident she will continue to achieve great things in whatever she pursues.",
    name: "Braydon & John",
    role: "APEX Financing",
  },
  {
    quote:
      "Thank you! I was able to access the file without any issues. I truly appreciate the opportunity to work with you and look forward to more collaborations in the future.",
    name: "Ismael Salic",
...
/** Tooltip rendered in a Portal so it never goes behind stacking contexts */
function TooltipPortal({
  ...
  return createPortal(
    <div
      ...
      role="tooltip"
    >
      <div className="relative w-max max-w-[280px] rounded-xl border border-white/10 bg-black/80 px-3 py-2 text-xs text-white/95 shadow-[0_18px_60px_rgba(0,0,0,.45)] backdrop-blur">
        <span className="block whitespace-normal break-words leading-snug">
          {text}
        </span>
        <span className="absolute left-1/2 top-0 h-2 w-2 -translate-x-1/2 -translate-y-1 rotate-45 border-l border-t border-white/10 bg-black/80" />
      </div>
    </div>,
    document.body
  );
...
type SkillItem = { category: string; name: string };
...
function SkillCard({
  ...
  item: SkillItem;
  onShow: (text: string, rect: DOMRect) => void;
  onHide: () => void;
  tipOpen: boolean;
}) {
  ...
  const showTip = () => {
    ...
    onShow(item.name, el.getBoundingClientRect());
  };

  const hideTip = () => onHide();
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div
            ref={labelRef}
            className="truncate text-[15px] font-medium text-white/90"
            title={item.name}
            tabIndex={0}
            onFocus={showTip}
            onBlur={hideTip}
...
export default function Home() {
  ...
  const showTip = (text: string, rect: DOMRect) => {
    setTipText(text);
    setTipRect(rect);
    setTipOpen(true);
  };
          ...
          {testimonials.map((t, idx) => (
            ...
            >
              <div className="flex items-start gap-3">
                <Quote size={18} className="mt-1 shrink-0 text-white/70" />
